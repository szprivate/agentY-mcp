...
2. **Use FP8/FP16 quantized models**: FP8 Flux models use ~8GB vs ~24GB for FP16
   - Search for FP8 variants: `search_huggingface_models("flux fp8")` or `search_huggingface_models("sdxl fp8")`
...
1. **Search for the node pack**:
   ```
   search_nodes("NodeClassName")
   ```
   Note: `search_nodes` searches installed ComfyUI nodes by keyword.
...
3. **Check logs for import errors**:
   ...
   ```
   Returns filtered lines from the ComfyUI server log.
...
1. **Check output indices**: Use `get_node_schema` to verify the exact output order
...
3. **Re-download**: If hash mismatch or corruption:
   ```
   download_hf_model(repo_id="...", filename="...", subfolder="checkpoints")
   ```
...
1. **Check current versions**:
   ```
   get_system_stats()  # Returns GPU info, VRAM, Python + PyTorch versions
   ```
...
```
get_history()                           # Most recent execution(s)
get_prompt_status_by_id("abc-123")      # Specific execution by prompt_id
```
...
queue(action="status")                  # Running and pending jobs
get_logs(keyword="error", max_lines=50) # Recent error logs
```

### Verify Node Availability

```
get_node_schema("KSampler")              # Check specific node schema
get_node_schema("ControlNetApply")       # Verify custom nodes loaded
```
...
| `Prompt outputs failed validation` | Node inputs don't match schema — check `get_node_schema` |
